...
__global__ void kernel_edge_count(
    unsigned int *flatten_doublets, 
    unsigned int *input_hits_chunk_idx,
    unsigned int *edge_count, // could use int here, but same vector will be used to store cumsum
    unsigned int n){
...
__global__ void kernel_triplet_iter_count(
    unsigned int *flatten_triplets, 
    unsigned int *input_hits_chunk_idx,
    ...
    for (int i=tid; i<n; i+=total_threds){
        ...
        iter_count[i] = (m1_end_idx - m1_start_idx) * (m2_end_idx - m2_start_idx) * (m3_end_idx - m3_start_idx);
    }
}

void launch_kernel_triplet_iter_count(
    unsigned int *flatten_triplets, 
    unsigned int *input_hits_chunk_idx,
    unsigned int *iter_count,
    unsigned int n,
    ...
    kernel_triplet_iter_count<<<grid_size, block_size>>>(flatten_triplets, input_hits_chunk_idx, iter_count, n);
}
...
__global__ void kernel_apply_doublet_cuts(
    unsigned int *flatten_doublets, 
    unsigned int *input_hits_chunk_idx,
...
//     - Not taking advantage of the doublet cuts (run the doublet cuts again and again)
...
__global__ void kernel_make_edges(
    ...
    unsigned long int tid = offset + (unsigned long int)blockDim.x * (unsigned long int)blockIdx.x + (unsigned long int)threadIdx.x;
    ...
    unsigned int n_hits_m1 = input_hits_chunk_idx[m1_idx + 1] - input_hits_chunk_idx[m1_idx];
    unsigned int n_hits_m2 = input_hits_chunk_idx[m2_idx + 2] - input_hits_chunk_idx[m2_idx];
    unsigned int n_hits_m3 = input_hits_chunk_idx[m3_idx + 3] - input_hits_chunk_idx[m3_idx];
...
void launch_kernel_make_edges(
    ...
    kernel_make_edges<<<grid_size, block_size, shared_mem_size, stream>>>(
        flatten_triplets, flatten_t2d_links, edge_count, input_hits, input_hits_chunk_idx, srcs, dsts, m_triplet_idx, offset);
